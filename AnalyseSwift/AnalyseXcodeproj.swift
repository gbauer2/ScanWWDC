...
public func analyseXcodeproj(url: URL, goDeep: Bool, deBug: Bool = true) -> (String, XcodeProj) {   //60-102 = 42-lines
    xcodeProj = XcodeProj()
    xcodeProj.url = url
...
//MARK: pbxToXcodeProj 323-lines
func pbxToXcodeProj(_ xcodeprojRaw: String, deBug: Bool = true) {        //105-428 = 323-lines
    if deBug {
    ...
    for ptrChar in 0..<chars.count {    //122-266 = 144-lines
        ...
        } else if char == ";" {                             // ";" 142-264 = 122-lines
            // uses: depth, bufrs[], deBug
            // calls: PBX.setDictPropertyPBX()
            var parts = [String]()
            var start = 1
...
private func gotSemicolon(depth: Int, bufrs: [String], deBug: Bool) {
...
func cleanRawText(_ rawText: String) -> (String, [Int]) {       //499-556 = 57-lines
    var pStart = 1
